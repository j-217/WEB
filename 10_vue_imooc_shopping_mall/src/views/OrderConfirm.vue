<template>
  <div>
    ...
    <nav-bread-crumb>
      <span slot="pagePath">OrderConfirm</span>
    </nav-bread-crumb>
    <div>
        ...
        <div class="checkout-order">
            ...
            <div class="price-count">
              <ul>
                <li>
                  ...
                  <span>{{ itemsTotalPrice | currency('¥') }}</span>
                </li>
                <li>
                  <span>Freight:</span>
          ...
          <div class="order-foot-wrap">
            ...
            <div class="next-btn-wrap">
              <a class="btn btn--m btn--red" @click="payOrder">Proceed to payment</a>
            </div>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      getOrderConfirmInfo(){
        ...
        axios.get('/users/getOrderConfirmInfo', {params: {addressId: addressId}})
          .then((response)=>{
            let res = response.data;
            if(res.status === '0'){
              this.addressId = res.result.addressId;
              this.goodsList = res.result.goodsList;
              this.itemsTotalPrice = res.result.itemsTotalPrice;
              ...
              console.log("获取订单确认信息失败")
            }
          })
      },

      payOrder(){
        axios.post('/users/payOrder', {
